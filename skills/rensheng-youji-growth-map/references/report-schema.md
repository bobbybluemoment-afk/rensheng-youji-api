...
`current_question_narrative` 使用关注领域中不超过3条阶段、挑战和应对判断，避免复制完整领域章节；通常按缩短模式写2—3段、320—500字，证据仅剩一条时按最小模式写1—2段、180—320字，证据缺口写1段、60—180字。它记录判断来源，并包含0—1条结构化重点句。证据允许时内容包括：
...
校准后状态为 `weakened`、`reject` 或 `uncertain` 的判断不得进入报告选材，也不得成为必进原句；`match` 直接写现实表现，`conditional/partial` 只按条件表达。正文不得解释用户选择了什么或报告为何排除某条判断。
