...
- 五题归一化后的用户可见A/B题意必须各不相同；只改时间范围、候选类型、内部问题轴或调换A/B顺序仍算同一道题；
- 证据较弱候选不得为了凑够五题进入题单。
...
`build_calibration_questions.py` 对候选确定性评分，优先顺序为待校准判断、条件判断、独立补充、阶段判断、主要判断，并对用户关注领域适度加分。程序枚举满足覆盖条件且用户可见A/B题意不重复的五题组合，再选总分最高的一组；相同输入永远得到相同五题。
...
问题文件使用 `schema_version=3.0.0`、`template_version=2.0.0`，并由程序写入 `source.analysis_id` 和 `source.baseline_sha256`。Core冻结前、选择候选时和问题生成后共用同一份现实可回答性与可见题意去重契约，检查未来年份、建议型答案、过长时间窗、多问题捆绑、领域漂移和A/B题意重复。校验器还会重新从同一个Core计算哈希、显示内容、候选绑定、答案影响、领域覆盖和时间题覆盖。内部候选编号、判断编号、命理证据和置信度不得进入 `calibration-visible.md`。
