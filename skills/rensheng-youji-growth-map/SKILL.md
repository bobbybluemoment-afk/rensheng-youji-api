...
- 校验失败时只允许重新运行出错的正式阶段一次。仍失败则按“交付容错”降级，不得伪造字段让校验器放行。
...
6. 正式报告使用 `schema_version=2.13.0`、`document_mode=full_calibrated`。Core使用0.11.0、事实提纲和初稿使用1.5.0、中文编辑使用2.4.0。Core先让规定方法家族独立完成技术推演和现实候选；单个方法最多独立重试3次，仍失败则退出综合投票。只有方法覆盖度达到完整或降级交付门槛时才能冻结Baseline并继续正式报告，`preliminary_only` 必须停止正式链路。完整人生主线先根据全盘材料生成，再在第4页单独回应用户问题。六个领域先写各自的人物侧面，再用人生主线串联；用户关注方向只在当前阶段、问题回应、相关年度和行动建议中加重。
