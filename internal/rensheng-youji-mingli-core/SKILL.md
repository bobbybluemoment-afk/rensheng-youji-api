...
生成8—15个可证伪现实候选，逐条标注固定领域、候选类型、时间范围和受影响的结论路径。报告层再通过固定题型把候选组织成同一比较轴下的A/B/C。吸收用户选择、D“都不符合／不确定”和补充事实时，只更新对应现实映射、候选排序与置信度。不得倒改四柱和结构事实，不得把未选择候选换词包装成正确。
...
27. `reality_candidate_pool`
    - 8—15条可验证现实候选；每条必须包含固定 `domain`、`candidate_kind`、`time_scope`、`calibration_targets`、2—3个可观察例子和一个真正不同的替代解释；候选需覆盖至少四个生活领域，不使用“务实、稳定、有责任感”等宽泛标签代替现实行为；
...
完整输出契约见 [analysis-output.schema.json](schemas/analysis-output.schema.json)。当前 `core_version` 使用 `0.4.0`。完成分析后运行：
