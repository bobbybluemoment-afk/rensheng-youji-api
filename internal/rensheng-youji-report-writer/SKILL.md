...
3. `delivery_mode=normal` 的完整人生主线写2—4个自然段，共500—700个汉字，完整综述性格、形成、资源、事业财富、关系、矛盾与阶段变化。
...
6. 每个内容区记录使用的判断编号、覆盖项和 `paragraph_claim_map`。正常和缩短章节每段至少对应两个Core判断，最小章节每段至少一个；证据缺口章节允许不引用判断，但只能说明暂不下结论。领域独立性与降级字段必须原样继承事实提纲。
