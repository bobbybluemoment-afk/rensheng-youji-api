...
AI只读取这份任务包，逐一填写 `slot_id` 对应的自然中文段落，并填写2—4项能力与资源、当前阶段、3—6个连续时间阶段、3—8个重点年份、行动和2—5个待观察问题。完整20年逐年数据由冻结Core和卡片链保留，写作AI不得重复生成20份年度文字。完整人生主线必须沿Core的“核心模式→原有能力→后来形成的做法→当前代价→领域表现→发展方向”组织；六领域分别使用本领域真实生活语言。不得再生成最终报告不会使用的重复摘要，不得读取原始四柱重新推命。
3. 完整人生主线继承事实提纲的 `delivery_mode`：`normal` 写2—4段、500—700字，`shortened` 写2—3段、320—500字，`minimal` 写1—2段、180—320字，`evidence_gap` 写1段、60—180字。当前问题另按压缩回答契约处理，不因上游标记为 `normal` 就扩写成第二篇领域综述。
...
写作层只负责组织人物材料，不负责数据库式来源登记或最后中文润色。不得用固定“好处—代价—建议”模板，不显示校准结果，不新增Core没有授权的行业、岗位、经历、关系状态或年份。`calibration_status` 只用于决定语气：`match` 直接写现实表现，`conditional/partial` 写清条件；上游已排除的 `weakened/reject/uncertain` 不得补回。正文不得出现“你已经确认”“你没有确认”“选择A/B/C/D”或“因此报告不会”等问卷操作语言。

六领域负责解释长期模式；`current_question` 只回答当前阶段、现实取舍和下一步动作。当前问题使用上游压缩后的不超过3条判断，按缩短模式写2—3段、320—500字，不重复对应领域的完整能力画像。

20年结构化趋势继续完整交给K线。用户可见长文严格采用任务包中的阶段边界与重点年份，只解释这些区段，不得自行增删年份，不得为每年强造“评价结算、角色定型、重新归档”一类标题。事业章节优先使用Core允许的任务、岗位功能或组织环境例子；具体行业或岗位只能写成来源支持的候选。行动建议必须逐项对应前文已经解释的问题。
