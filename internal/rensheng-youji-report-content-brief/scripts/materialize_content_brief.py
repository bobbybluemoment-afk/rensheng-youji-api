...
def materialize(selection: dict[str, Any], analysis: dict[str, Any], resolved: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    is_v09 = meta.get("core_version") in {"0.9.0", "0.10.0", "0.11.0", "0.12.0", "0.13.0", "0.14.0", "0.15.0", "0.16.0", "0.17.0"}
    is_v081 = meta.get("core_version") == "0.8.1"
    is_v08 = meta.get("core_version") == "0.8.0"
    result["schema_version"] = "1.6.0" if meta.get("core_version") in {"0.15.0", "0.16.0", "0.17.0"} else "1.5.0" if is_v09 else "1.4.0" if is_v081 else "1.3.0" if is_v08 else "1.2.0"
    ...
    if is_v09:
        if not isinstance(resolved, dict):
            raise ValueError("core_version=0.9.0—0.17.0 必须先提供校准后确定性选材文件")
...
def main() -> int:
    ...
    try:
        ...
        elif analysis.get("analysis_meta", {}).get("core_version") in {"0.15.0", "0.16.0", "0.17.0"} and resolved:
