...
description: 人生有迹内部报告事实整理层。接收已校验且完成现实校准的0.5.0 Core母稿，从开放现实候选、判断台账、人物形成链与领域联动链中选择可进入完整报告的材料，生成不含用户文章的report-content-brief.json。用于完整报告写作前锁定事实、置信度、现实名词和关注方向范围；不负责排盘、重新推命、写正文或渲染PDF。
---
...
1. 读取校准后的 Core 母稿，确认 `core_version=0.5.0`。
...
6. 生成 `report-content-brief.json`，运行：
