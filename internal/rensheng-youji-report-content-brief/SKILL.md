...
description: 人生有迹内部报告事实整理层。接收已冻结并以校准增量合成的0.16.0 Core母稿和确定性校准后选材，从解释主线、现实细节、六领域独立判断、人物形成链与领域联动链中实体化证据、必须兑现的白话判断和校准变化，确定性生成不含用户文章的report-content-brief.json。用于完整报告写作前锁定事实与判断；不负责排盘、重新推命、AI选材、写正文或渲染PDF。
---
...
1. 读取校准后的 Core 母稿，确认 `core_version=0.16.0`、`method_execution_audit.delivery_decision` 不是 `preliminary_only`，并确认它已通过Baseline冻结校验。解释主线、现实细节编号和报告判断必须原样可追溯，不得压缩成几个抽象主题词。
...
- 每个内容区必须按 `delivery_mode` 原样携带0—2条 `mandatory_claims`：正常模式1—2条，缩短或最小模式1条，证据缺口模式0—1条；没有可靠判断时为0条，仍保留一条可靠判断时允许锁定1条。每项包括 `claim_id`、`plain_claim`、判断家族、机制家族和新增信息。后续写作必须逐字兑现 `plain_claim`；事实提纲不能自行概括。
