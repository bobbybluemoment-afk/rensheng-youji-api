...
description: 人生有迹内部报告事实整理层。接收已冻结并以校准增量合成的0.11.0 Core母稿和确定性校准后选材，从六领域独立判断、方法综合、人物形成链与领域联动链中实体化证据、必须兑现的白话判断和校准变化，生成不含用户文章的report-content-brief.json。用于完整报告写作前锁定事实与判断；不负责排盘、重新推命、自由选择报告结论、写正文或渲染PDF。
---
...
1. 读取校准后的 Core 母稿，确认 `core_version=0.11.0`、`method_execution_audit.delivery_decision` 不是 `preliminary_only`，并确认它已通过Baseline冻结校验。
