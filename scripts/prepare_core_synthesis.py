...
def compact_view(source: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "semantic_output_contract": source["semantic_output_contract"] | {
            ...
            "calibration_probe_rule": "Core只生成有分析价值的reality_candidate_pool，不生成validation_question或answerable_*。程序会把statement、alternative_statement与time_scope编译为校准探针，只让过去或当前可观察、单一现实问题轴且领域语言清楚的候选进入五题池；未来阶段、复合判断和不适合提问的候选继续保留在Core与报告素材中，不得为了出题删除。",
            ...
            "calibration_feasibility_rule": "冻结前由程序从reality_candidate_pool筛出的可提问子集必须能组成恰好5题：至少4个领域、同领域最多2题、5个不同现实问题轴、至少1条已发生且有时运依据的timed_event、timed_event与objective_state合计至少2条。并非每条Core候选都必须适合出题；未来阶段、复合判断和报告专用候选允许留在池中但不会被选择。",
