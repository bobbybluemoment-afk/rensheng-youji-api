...
class AiStageContractTest(unittest.TestCase):
    ...
    def test_missing_method_scaffold_is_detected(self) -> None:
        contract = copy.deepcopy(self.contract)
        stage = next(item for item in contract["stages"] if item["id"] == "independent_methods")
        stage.pop("scaffold")
        self.assertTrue(any("草稿生成器" in item for item in audit(ROOT, contract)))

    def test_missing_ai_output_contract_is_detected(self) -> None:
        contract = copy.deepcopy(self.contract)
        stage = next(item for item in contract["stages"] if item["id"] == "content_selection")
