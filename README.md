...
完整报告 Skill 同样免费，不需要验证码。五条现实校准由程序从当前Core候选中确定性选择并生成，AI不参与选题。每题只比较同一件事，至少两题核对客观状态或已发生事件，至少一题用明确时间窗口校准大运流年的现实执行。用户选择A/B/C时由程序直接处理；只有选择D并写自由答案时，才让AI把原话整理成受限的事实补丁。完成校准并通过出生时间边界预检后，固定交付新版人生卡片、Markdown和10页PDF；若跳过校准，只生成明确标注的“初步分析”和新版卡片。
...
完整人生主线写成3—4个连贯自然段，综合性格、家庭教育、事业财富、关系方式和当前阶段。六个生活领域分别写成3—4个自然段，每节500—700字，覆盖特征、行为方式、形成经历、现实环境、重复挑战、阶段变化和应对，但不把这些要求显示成固定小标题。报告先把Core结论、机制、证据和限制实体化，再完成人物初稿和可追溯中文编辑；每个自然段至少映射两个Core判断，校准答案只作修正。PDF正文固定使用仓库内置霞鹜文楷，统一左对齐和字号层级。
