...
两者使用同一套确定性排盘和命理Core，不会分别生成互相矛盾的人生主线。Core会强制格局、气势意向、调候、十神网络、根苗花果、盲派和岁运连续性分别完成技术推演与现实候选，再由综合层判断哪些方向得到多个独立方法支持、哪些只是有条件的单方法补充。神煞和纳音只作确定性辅助，不能单独成为报告判断。
...
完整报告 Skill 同样免费，不需要验证码。五条现实校准使用固定题型库：用户模型只选择最有命理校准价值的比较轴并绑定Core候选，题干和A/B/C由程序确定性生成。每题只比较同一件事，至少两题核对客观状态或已发生事件，至少一题用明确时间窗口校准大运流年的现实执行。完成校准并通过出生时间边界预检后，固定交付新版人生卡片、Markdown和10页PDF；若跳过校准，只生成明确标注的“初步分析”和新版卡片。
...
确定性排盘
  ...
  └→ 五条现实校准 → 实体化事实提纲 → 人物初稿 → 中文编辑 → 同一新版卡片 + 固定10页PDF
```
