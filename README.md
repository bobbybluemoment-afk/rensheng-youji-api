...
两者使用同一套确定性排盘和命理Core，不会分别生成互相矛盾的人生主线。Core会强制格局、气势意向、调候、十神网络、根苗花果、盲派、岁运连续性、宫位六亲和干支根气分别完成技术推演与现实候选，再由综合层判断哪些方向得到多个独立方法支持、哪些只是有条件的单方法补充。当前流程不使用神煞和纳音。
...
AI 只需询问出生年月日时、出生城市和性别；姓名可以不填，时间默认按普通钟表时间处理。成功后返回一张1242×1660 PNG卡片。

### 生成完整报告

请让AI读取本仓库的 `skills/rensheng-youji-growth-map/SKILL.md`，或发送：

> 请使用“人生有迹完整报告”Skill，先询问我的出生日期、准确时间、出生城市、性别和关注问题，再经过五条事实校准，为我生成完整报告。

完整报告 Skill 同样免费，不需要验证码。五条现实校准由程序从当前Core候选中确定性选择并生成，AI不参与选题。每题只比较同一件已经发生或当前能够观察的事，禁止要求用户判断未来几年会怎样；至少两题核对客观状态或已发生事件，至少一题使用截至当前的明确时间窗口。用户选择A/B/C时由程序直接处理；只有选择D并写自由答案时，才让AI把原话整理成受限的事实补丁。完成校准并通过出生时间边界预检后，固定交付新版人生卡片、Markdown和10页PDF；若跳过校准，只生成明确标注的“初步分析”和新版卡片。

报告采用两轮简化输入：第一轮只收出生时间、出生地、性别以及关注方向或具体问题；第二轮让用户一次回复五道校准题。事业或学习问题中的学历、专业、职业，感情或家庭问题中的关系与家庭状态，以及其他方向的现实资料都只是可选补充。用户不回答这些资料也会继续生成，不会要求逐项填写“不知道”。

报告封面使用固定产品介绍和“人生有迹”Logo，不再重复卡片结论。用户选择的关注方向只在“当前阶段与问题回应”、相关年度提醒和行动建议中重点展开；完整人生主线、能力形成和六个生活领域仍从全盘出发，避免整份报告被单一问题带偏。

Core先保留九方法中的现实细节，再形成“核心模式—原有能力—后来做法—当前代价—领域表现—发展方向”的解释主线。完整人生主线沿这条链写成连贯自然段；六个领域是同一个人在不同生活场景里的具体表现，不再使用同一套项目管理语言。报告遵守《人生有迹自然中文写作标准》，一句只说一个主要意思，抽象判断必须翻译成现实动作、感受或关系场景；校准答案只作修正。PDF正文固定使用仓库内置霞鹜文楷，统一左对齐和字号层级。

报告会把判断落到可以核对的现实层面。事业先分别判断工作方式、组织属性、行业、部门、岗位、处理对象和发展方式，再从公务员、事业单位、国企、民企、外企、专业机构、自由职业、创业等开放候选中选择有证据的少数方向；人力、财务、法务、IT技术、研究、项目、运营、销售等岗位同样不设封闭词库。财富、家庭和伴侣也采用开放候选体系。每个具体名词都有Core交叉证据或用户事实，证据不足时只写到类别边界。重点判断采用少量独立深青色行；若视觉增强不能稳定排版，交付程序会自动切换到统一正文的稳定模式，不影响可靠内容交付。
...
确定性排盘
  → 程序生成九份隔离短提示
  → 九方法并行或分组生成精简语义答卷
  → 程序编译正式方法包并执行METHOD GATE
  → 按有效方法覆盖度决定完整、降级或仅初步分析
  → 程序把方法包压成六领域判断矩阵并冻结完整编译源
  → AI完成受约束语义综合
  → 程序组装完整Core并生成报告来源
  → Core现实综合与完整分析
  ├→ 确定性卡片文字 + Core逐年结构映射 → 共享2.0.0数值算法 → 新版PNG绘图
  └→ 程序选择五条现实校准 → 确定性事实提纲 → AI写正文 → 按需局部编辑 → 调用同一卡片算法 + 固定10页PDF
```
