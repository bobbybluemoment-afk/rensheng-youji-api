...
六个生活领域分别按自己的内容结构展开，写清特征、行为方式、形成经历、现实环境、重复挑战、阶段变化和应对，不再套用统一的“判断—好处—代价”格式。报告初稿还会经过事实保留式中文编辑，禁止直接复制校准选项或使用生硬内部标签。PDF正文固定使用仓库内置霞鹜文楷，统一左对齐和字号层级。

报告会把判断落到可以核对的现实层面：每个领域先给一个主判断，再写现实名词、重复表现和一个核对点。事业先判断工作机制，再在证据允许时落到大型国企、成熟科技公司、项目管理等组织与岗位名词；财务写固定工资、绩效、项目奖金或回款方式；关系写见面频率、城市和金钱安排；家庭写住房、教育费用与照护分工。每个名词都要有Core交叉证据或用户事实，证据不足时只写到类别边界，不为了明确而猜唯一答案。
